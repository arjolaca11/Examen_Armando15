...
public class ControlGastosGUI extends JFrame {
    ...
    // ---- Pedir datos en bucle (sin try/catch) ----
        ...
        while (ok == false) {
            if (valor != null) {
                ...
                if (esNombreValido(valor) && existeNombre(valor)==false) ok = true;
                    ...
                    error("Dato inválido", "Nombre inválido o repetido (solo letras y espacios).");
    ...
    // ====== NUEVO: reporte con RESUMEN TOTAL por persona ======
    private String reporteDeudasDetalladasPorMovimiento(){
        ...
        double[][] matriz = sistema.calcularMatrizDeudas();  // usa tu método del modelo
    ...
    private void agregarMovimiento(){
        ...
        if (indicesParticipantes == null) return;
  ...
  private void verCuentas(){
    ...
    // Anchos ajustados (ya no existe la columna "Moneda")
    tabla.setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
    ...
    // Puedes dejar el título aclarando la moneda
